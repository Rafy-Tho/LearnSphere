...
| Ops | Migrations applied to the live DB | ✅ | `0001`–`0017` applied (`npm run db:status`). `0013` adds email verification; `0014` adds `user_auth_providers` + nullable `users.password`; `0015` adds `quiz_attempts` + `quiz_answers`; `0016` adds `saved_courses`; `0017` adds `user_activities` + `user_xp_transactions`, backfills lesson XP, renames `learn_progress.lesson_id` → `current_lesson_id`, drops `lesson_completion.time_spent_minutes`/`xp_earned`, and adds `certificates.confirmed_at`. |
