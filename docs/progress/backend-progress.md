...
| Ops | Migrations applied to the live DB | ✅ | `0001`–`0021` applied (`npm run db:status`). `0018` adds coupons/redemptions/refunds/webhook-events, billing columns, `plan_id` → RESTRICT, and indexes; `0019` backfills payment subtotal/paid_at; `0020` removes recurring columns/indexes and adds plan `features` + webhook `status`; `0021` backfills plan features. Baseline plans seeded via `db:seed` (`000_subscription_plans.sql`). Note: `db:seed` currently stops at `055` (pre-existing duplicate `unique_quizzes_lesson_position`), leaving `055`–`058` pending. |
